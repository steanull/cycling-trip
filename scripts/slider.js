...
const tabsContentBicycles = document.querySelectorAll('.bicycles__slider');
...
document.addEventListener('click', (e) => {
  ...
  if (e.target.classList.contains('coverage__slider-button_control_prev')) {
  ...
  if (e.target.classList.contains('coverage__slider-button_control_next')) {
    let activeBtn = document.querySelector('.bicycles__link_active');
    let activeParent = activeBtn.closest('.bicycles__menu-item');
...
const tabsHandler = (path) => {
  tabsContentBicycles.forEach(el => {
    el.classList.remove('bicycles__slider_active')
  });
  tabsContentCoverage.forEach(el => {
    el.classList.remove('coverage_active')
  });

  document.querySelector(`[data-tabs-target="b-${path}"]`).classList.add('bicycles__slider_active');
